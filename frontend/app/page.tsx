...
export default function Home() {
  ...
  const submitAuth = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try { await api(`/auth/${authMode}`, { method: "POST", body: JSON.stringify(body) }); await load(); }
    ...
    try {
      ...
      while (true) {
        ...
        for (const event of events) {
          ...
          try { const obj = JSON.parse(payload); const delta = obj.choices?.[0]?.delta?.content || ""; if (delta) setMessages((m) => { const copy = [...m]; copy[copy.length - 1] = { ...copy[copy.length - 1], content: copy[copy.length - 1].content + delta }; return copy; }); } catch {}
        }
      }
    } catch (err: any) { setMessages((m) => { const copy = [...m]; copy[copy.length - 1] = { role: "assistant", content: `خطا: ${err.message}` }; return copy; }); }
  ...
  const createKey = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const data = await api("/api-keys", { method: "POST", body: JSON.stringify({ alias: fd.get("alias"), models: [selectedModel], rpm_limit: Number(fd.get("rpm") || 30), duration: fd.get("duration") || "30d" }) });
      setNewKey(data); setKeyModal(false); await load();
    } catch (err: any) { setFormError(err.message); }
  };
  ...
  const sidebar = useMemo(() => [
    ["dashboard", "داشبورد", "home"], ["chat", "چت", "chat"], ["keys", "کلیدهای API", "key"], ["usage", "مصرف و Usage", "chart"], ["account", "حساب کاربری", "user"],
  ], []);
  ...
  if (!user) return (
    <main className="auth-shell">
      <section className="auth-card">
        <div className="brand-lockup"><div className="brand-mark large">H</div><div><b>Hinaa</b><span>هوش مصنوعی حرفه‌ای</span></div></div>
        <h1>{authMode === "login" ? "خوش آمدید" : "ساخت حساب کاربری"}</h1>
        <p className="muted">دسترسی به چت و سرویس‌های هوش مصنوعی Hinaa</p>
        <form onSubmit={submitAuth} className="form-stack">
          {authMode === "register" && <label>نام<input name="name" required placeholder="نام شما" /></label>}
          <label>ایمیل<input name="email" type="email" required placeholder="you@example.com" /></label>
          <label>رمز عبور<input name="password" type="password" minLength={8} required placeholder="حداقل ۸ کاراکتر" /></label>
          {formError && <div className="error-box">{formError}</div>}
          <button className="primary full" type="submit">{authMode === "login" ? "ورود" : "ثبت‌نام"}</button>
        </form>
        <button className="link-btn" onClick={() => { setAuthMode(authMode === "login" ? "register" : "login"); setFormError(""); }}>{authMode === "login" ? "حساب ندارید؟ ثبت‌نام کنید" : "حساب دارید؟ وارد شوید"}</button>
      </section>
    </main>
  );
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <button className="new-chat" onClick={() => { setActive("chat"); setMessages([]); }}><Icon name="plus"/>گفتگوی جدید</button>
      ...
      <section className="main-panel">
        ...
        {active === "dashboard" && <Dashboard user={user} data={dashboard} keys={keys} onChat={() => setActive("chat")} onKeys={() => setActive("keys")} />}
        {active === "chat" && <Chat selectedModel={selectedModel} setSelectedModel={setSelectedModel} models={models} messages={messages} input={input} setInput={setInput} sendChat={sendChat} sending={sending} />}
        {active === "keys" && <Keys keys={keys} models={models} selectedModel={selectedModel} setSelectedModel={setSelectedModel} createKey={createKey} deleteKey={deleteKey} rotateKey={rotateKey} openModal={() => setKeyModal(true)} newKey={newKey} setNewKey={setNewKey} />}
        {active === "usage" && <Usage keys={keys} data={dashboard} />}
